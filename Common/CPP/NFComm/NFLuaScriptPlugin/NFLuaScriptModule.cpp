...
struct LocalRegisterLuaFunction
{
	NFLuaScriptModule* p;
};
...
bool NFLuaScriptModule::Register()
{
	...
	LuaIntf::LuaBinding(mLuaContext).addFunction("IsFileExist", [this](std::string path) {
		...
		.addFunction("GetFilePath", [this](const std::string& path) {
		return m_pFileSystemModule->GetFilePath(path);
	})
.addFunction("FileWriteTime", [this](const std::string& path) {
		return m_pFileSystemModule->FileWriteTime(path);
	})
.addFunction("GetFolderFiles", [this](const std::string& path, bool recursive) {
		auto list = m_pFileSystemModule->GetFolderFiles(path, recursive);
		LuaIntf::LuaRef tbl = LuaIntf::LuaRef::createTable(mLuaContext.state());
...
.addFunction("GAddClass", [L] {
 ...
 ;
	...
	LuaIntf::LuaBinding(mLuaContext).beginClass<LocalRegisterLuaFunction>("LuaFilePicker")
		//.addMetaFunction("IsFileExist", [](std::string path){
		//		return m_pFileSystemModule->IsFileExist(path);
		//	})
		//.addMetaFunction("GetFilePath", [this](const std::string& path) {
		//		return m_pFileSystemModule->GetFilePath(path);
		//	})
		//.addMetaFunction("FileWriteTime", [this](const std::string& path) {
		//		return m_pFileSystemModule->FileWriteTime(path);
		//	})
		//.addMetaFunction("GetFolderFiles", [this](const std::string& path) {
		//		return m_pFileSystemModule->GetFolderFiles(path);
		//	})
