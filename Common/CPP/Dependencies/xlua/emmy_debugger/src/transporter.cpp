...
void Transporter::Receive(const char* data, size_t len)
{
	...
	receiveSize += len;
	std::cout << "dbg: Receive" << len << std::endl;
	size_t pos = 0;
	while (true)
	{
		size_t start = pos;
		...
		if (start != pos)
		{
			...
			else
			{
				...
				char* tbuf = new char[pos - start];
				memset(tbuf, 0, pos - start);
				memcpy(tbuf, buf + start, pos - start - 1);

				std::cout << "dbg: Receive parse Begin" << pos - start << std::endl;

				//document.Parse(buf + start, pos - start);
				document.Parse(tbuf);
				std::cout << "dbg: Receive parse End" << pos - start << std::endl;
