...
local battle_instance_turned = class2("battle_instance_turned", function(self, battle_id, init_data)
    self._bid = battle_id
    self._mods = {}
...
function battle_instance_turned:new_oid()
    self._oid_idx = self._oid_idx + 1
    return self._oid_idx
end

function battle_instance_turned.init()
end

function battle_instance_turned:make_log_str()
    self._log_str = string.format("bid=%s <color='#FFAA22'>%d回合</color>", self._bid, RPG_I2F(self:get_btime()))
end

function battle_instance_turned.create_main_level_data(level_id, player)
    -- return init_data {}
end

function battle_instance_turned:log_str()
...
function battle_instance_turned.create_battle(battle_id, param)
    ...
    --     battle_instance_turned.create_client_battle(rpg_ins, param)
...
function battle_instance_turned:listening(event_id)
    local list = self._evt_listeners[event_id]
    return list and next(list) and true 
end

function battle_instance_turned:add_event_listener(event_id, func)
    self._evt_listeners[event_id] = self._evt_listeners[event_id] or {}
    table.insert(self._evt_listeners[event_id], func)
end

function battle_instance_turned:add_event_listener2(event_id,listener,func_name)
...
function battle_instance_turned:remove_event_listener(event_id, func)
    local list = self._evt_listeners[event_id]
    if list then
...
function battle_instance_turned:remove_event_listener2(event, listener,func_name)
...
function battle_instance_turned:bpause(pause)
    self._bpause = pause
end

function battle_instance_turned:post_event2(event_id, event)
...
function battle_instance_turned:post_event(event)
    event.rtime = self:get_rtime()
...
function battle_instance_turned:add_action_listener(action, func)

end

function battle_instance_turned:remove_action_listener(action, func)

end

function battle_instance_turned:post_action(action)
end


function battle_instance_turned:add_mod(mod)
    table.insert(self._mods, mod)
end


function battle_instance_turned:start()
    RPG_LOG("[RPG]%s battle_instance_turned start", self:log_str())
...
-- 设置战斗播放速度
function battle_instance_turned:set_speed(speed)
...
function battle_instance_turned:stop()
    if self._finish then
        RPG_ERR("[RPG]%s battle_instance_turned already finish", self:log_str())
        return
    end
    for k, mod in ipairs(self._mods) do
    ...
    self._finish = true
    RPG_LOG("[RPG]%s battle_instance_turned stop", self:log_str())
end

function battle_instance_turned:init_phase()
...
function battle_instance_turned:pause()
    self._pause = true
end

function battle_instance_turned:resume()
    self._pause = false
end

function battle_instance_turned:add_fixed_update(update, order_name)
    if order_name then
...
---50
--- 5000 / 50
function battle_instance_turned:update(dt_ms)
    if self._finish then
        return
    end
    ...
    -- RPG_DEBUG("[RPG]%s battle_instance_turned update time=%s _fixed_time=%s", self:log_str(), self._time, self._fixed_time)
...
-- 跳过战斗，直接结算
function battle_instance_turned:skip()
    self._skip = true
...
-- 获取战斗结束倒计时时间
function battle_instance_turned:get_count_down_time()
