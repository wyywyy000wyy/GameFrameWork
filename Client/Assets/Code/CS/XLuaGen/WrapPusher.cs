...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderLuaBridgeType
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<NetEventType>(translator.PushNetEventType, translator.Get, translator.UpdateNetEventType);
        ...
        int NetEventType_TypeID = -1;
		int NetEventType_EnumRef = -1;
        
        public void PushNetEventType(RealStatePtr L, NetEventType val)
        {
            if (NetEventType_TypeID == -1)
            {
			    bool is_first;
                NetEventType_TypeID = getTypeId(L, typeof(NetEventType), out is_first);
				
				if (NetEventType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(NetEventType));
				    NetEventType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, NetEventType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, NetEventType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for NetEventType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, NetEventType_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out NetEventType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != NetEventType_TypeID)
				{
				    throw new Exception("invalid userdata for NetEventType");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for NetEventType");
                }
				val = (NetEventType)e;
            ...
            {
                val = (NetEventType)objectCasters.GetCaster(typeof(NetEventType))(L, index, null);
            }
        }
		
        public void UpdateNetEventType(RealStatePtr L, int index, NetEventType val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for NetEventType ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(NetEventType[]))
			{
			    NetEventType[] array = obj as NetEventType[];
				translator.PushNetEventType(L, array[index]);
				return true;
			}
