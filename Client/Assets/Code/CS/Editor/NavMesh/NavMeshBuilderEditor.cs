...
{
    public class GeomProvider : IInputGeomProvider
    {
        ...
        public static GeomProvider LoadFile(string objFilePath)
        {
            byte[] chunk = RcResources.Load(objFilePath);
            var context = RcObjImporter.LoadContext(chunk);
            return new GeomProvider(context.vertexPositions, context.meshFaces);
        }

        //public static GeomProvider LoadFile()
        //{

        //}

        public GeomProvider(List<float> vertexPositions, List<int> meshFaces) :
            this(MapVertices(vertexPositions), MapFaces(meshFaces))
        {
        }

        public GeomProvider(float[] vertices, int[] faces)
        {
    ...
    {
        GeomProvider provider = GeomProvider.LoadFile("Assets/Models/NavMesh.obj");
        RcNavMeshBuildSettings _settings = new RcNavMeshBuildSettings();
        TileNavMeshBuilder tileNavMeshBuilder = new TileNavMeshBuilder();
        NavMeshBuildResult result = tileNavMeshBuilder.Build(provider, _settings);

        var navmeshFilePath = $"test.navmesh";

        var navMesh = result.NavMesh;
